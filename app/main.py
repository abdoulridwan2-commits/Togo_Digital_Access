...
REGION_MAP = {
    "Maritime": ["Bas-Mono", "Golfe", "Lacs", "Lome Commune", "Vo", "Yoto", "Zio"],
    "Plateaux": ["Agou", "Akébou", "Amou", "Anié", "Ave", "Danyi", "Est-Mono", "Haho", "Kloto", "Kpélé", "Moyen-Mono", "Ogou", "Wawa"],
...
PREFECTURE_TO_REGION = {prefecture: region for region, prefectures in REGION_MAP.items() for prefecture in prefectures}
...
def section_conclusion(text):
    st.markdown(f'<div class="conclusion"><strong>📌 Conclusion de la section</strong><br>{text}</div>', unsafe_allow_html=True)
...
def load_data():
    ...
    data["region"] = data["prefecture"].map(PREFECTURE_TO_REGION)
    return data
...
def load_boundaries():
    ...
    boundaries["region"] = boundaries["prefecture"].map(PREFECTURE_TO_REGION)
...
missing_regions = sorted(df.loc[df.region.isna(), "prefecture"].tolist())
if missing_regions:
    st.error(f"Préfectures sans région configurée: {', '.join(missing_regions)}")
    st.stop()

with st.sidebar:
    st.markdown("### Périmètre d'analyse")
    selected_region = st.selectbox("Région du Togo", ["Toutes"] + list(REGION_MAP), key="global_region")
    if selected_region == "Toutes":
        st.caption(f"{len(df)} préfectures · 5 régions")
    else:
        st.caption(f"{int((df.region == selected_region).sum())} préfecture(s) affichée(s)")
...
with tabs[0]:
    ...
    top3_view = top3[["rang_affiche", "prefecture", "score_priorite", "niveau_priorite", "population_worldpop", "agents_mm_pour_10000_hab", "antennes_opencellid"]].copy()
    ...
    displayed_top = top.head(10)
    interpretation(f"Classement {signal_scope}", f"Les {len(displayed_top)} scores affichés vont de {fmt_decimal(displayed_top.score_priorite.min(), 3)} à {fmt_decimal(displayed_top.score_priorite.max(), 3)}.", "Le score sert à concentrer les moyens d'analyse sur un périmètre prioritaire avant les décisions d'investissement.")
    section_conclusion(f"Le territoire le plus prioritaire dans le périmètre {signal_scope} est {top.iloc[0].prefecture}. Ce résultat sert à orienter la validation terrain et les investissements numériques.")
...
with tabs[1]:
    ...
    map_data = boundaries.drop(columns=["region"], errors="ignore").merge(df, on="prefecture", how="left")
    ...
    region_boundaries = boundaries.dissolve(by="region", as_index=False)
    ...
    folium.GeoJson(
        region_boundaries.to_json(),
        name="Limites des régions",
        style_function=lambda feature: {"fillOpacity": 0, "color": "#102a43", "weight": 2.4, "dashArray": "7, 5"},
        tooltip=folium.GeoJsonTooltip(fields=["region"], aliases=["Région"], labels=True),
    ).add_to(m)
    ...
    fields = ["prefecture", "region", "population_worldpop", "agents_mobile_money", "agents_mm_pour_10000_hab", "antennes_opencellid", "score_priorite", "niveau_priorite"]
    aliases = ["Préfecture", "Région", "Population", "Agents Mobile Money", "Agents / 10 000 hab.", "Antennes observées", "Score", "Priorité"]
    if not map_data.empty:
        ...
        st.info(f"Aucune préfecture ne correspond au filtre « {selected_level} » dans le périmètre sélectionné.")
    ...
    if show_points:
        for name, filename, color, label in [("Agences Moov", "agences_moov.csv", "#0b7285", "Agence Moov"), ("Agences Togocom", "agences_togocom.csv", "#f4b942", "Agence Togocom"), ("Datacenters", "datacenters.csv", "#7c3aed", "Datacenter")]:
            ...
            if points.empty or map_data.empty:
                continue
    ...
    map_key = f"territorial_decision_map_{selected_level}_{selected_pref}".replace(" ", "_")
    map_event = st_folium(m, width=None, height=620, key=map_key, returned_objects=["last_object_clicked"])
    ...
    section_conclusion(f"La carte contient {len(map_data)} préfecture(s), colorée(s) selon le niveau de priorité, avec les limites des régions et les couches ponctuelles disponibles. La zone affichée correspond au filtre {signal_scope}.")
...
with tabs[2]:
    ...
    section_conclusion(f"Le déficit Mobile Money est le principal signal d'action dans {signal_scope}. Les territoires en bas du classement doivent être examinés en priorité pour renforcer les agents et points de service.")
...
with tabs[3]:
    ...
    section_conclusion(f"La carte des antennes montre des signaux à vérifier, pas une preuve de couverture ou de non-couverture. Une validation auprès des opérateurs est indispensable avant toute décision.")
...
with tabs[4]:
    ...
    section_conclusion(f"Les infrastructures physiques sont concentrées dans quelques territoires. Les zones moins équipées peuvent être ciblées avec des solutions mutualisées et des services numériques de proximité.")
...
with tabs[5]:
    ...
    section_conclusion(f"Entre {pref_a} et {pref_b}, {winner.prefecture} doit être examiné en premier selon le score composite. La décision finale doit toutefois intégrer le coût, la faisabilité et la validation terrain.")
...
with tabs[6]:
    ...
    for _, row in top.head(5).iterrows():
        ...
        with st.container(border=True):
            st.markdown(f"### {int(row.rang_affiche)} · {row.prefecture}  <span class='{priority_class(row.niveau_priorite)}'>{row.niveau_priorite}</span>", unsafe_allow_html=True)
    ...
    section_conclusion(f"Les recommandations proposent un ordre d'action pour {signal_scope}. Elles doivent être confirmées par les opérateurs, les collectivités et une mission terrain avant investissement.")
